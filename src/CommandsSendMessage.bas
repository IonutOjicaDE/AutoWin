Attribute VB_Name = "CommandsSendMessage"
Option Explicit
Private Const MODULE_NAME As String = "CommandsSendMessage"

Private Declare PtrSafe Function SendMessage Lib "user32" Alias "SendMessageA" (ByVal hWnd As LongPtr, ByVal wMsg As Long, ByVal wParam As LongPtr, lParam As Any) As LongPtr


Public Enum WndMsg           'for SendMessage, wMsg
...
Public Enum ButtonMsg
  BM_GETCHECK = &HF0           'Retrieves the check state of a radio button or check box.
  BM_SETCHECK = &HF1           'Sets the check state of a radio button or check box.
  BM_GETSTATE = &HF2           'Retrieves the state (pressed, focused, etc.) of the button.
  BM_SETSTATE = &HF3           'Sets the state of the button (typically for visual feedback).
  BM_SETSTYLE = &HF4           'Changes the button's style at runtime.
  BM_CLICK = &HF5              'Simulates a mouse click on the button.
  ...
  BM_GETIMAGE = &HF6           'Retrieves the image (icon or bitmap) associated with the button.
  BM_SETIMAGE = &HF7           'Assigns an image (icon or bitmap) to the button.
End Enum


Public Enum BST_Status
  BST_UNCHECKED = &O0
  BST_CHECKED = &O1
  BST_INDETERMINATE = &O2
End Enum


' https://learn.microsoft.com/en-us/dotnet/api/microsoft.crm.unifiedservicedesk.dynamics.controls.nativemethods.winuserconstant?view=dynamics-usd-3
Public Enum MK_Buttons
  MK_LBUTTON = &H1
  MK_RBUTTON = &H2
  MK_MBUTTON = &H10
End Enum
...
Public Type MINMAXINFO 'WM_GETMINMAXINFO
...
Public Type COPYDATASTRUCT 'WM_COPYDATA & WM_CANCELJOURNAL
  dwData As LongPtr
  cbData As Long
  lpData As LongPtr
End Type
...
Public Function ActivateWindow(hWnd As LongPtr) As LongPtr
  ActivateWindow = SendMessage(hWnd, WM_ACTIVATE, WA_ACTIVE, 0)
End Function


Public Function SetFocus(hWnd As LongPtr) As LongPtr
  SetFocus = SendMessage(hWnd, WM_SETFOCUS, 0, 0)
End Function


Public Function Paint(hWnd As LongPtr) As LongPtr
  Paint = SendMessage(hWnd, WM_PAINT, 0, 0)
End Function
...
' http://www.jasinskionline.com/windowsapi/ref/b/bm_getcheck.html
Private Sub Check_Status_of_Checkbox(hWnd As LongPtr)
  Dim state As LongPtr ' checked state of the check box
  
  ' Find the checked state of Check1 and tell the user what it is.
  state = SendMessage(hWnd, BM_GETCHECK, ByVal CLng(0), ByVal CLng(0))
  Select Case state
  Case BST_CHECKED
